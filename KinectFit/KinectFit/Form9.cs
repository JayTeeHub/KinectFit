...
namespace KinectFit
{
    public partial class Form9 : Form
    {
        ...
        DataTable dt;
        String statement;
        public Form9(ConnectionClass _cc)
        {
            InitializeComponent();
            cc = _cc;
        }
        ...
        private void listBrands_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            statement = "select name from clothes WHERE gender = '" + cc.UserGender + "'AND size = '" + cc.UserSize + "' AND sizeQty >= 1 and type != '"+ cc.TrueStyle +"' AND brand = '" + listBrands.SelectedItem.ToString() + "';";
            dt = new DataTable();
            dt = cc.queryResults(statement);
            if (dt != null)
            {
