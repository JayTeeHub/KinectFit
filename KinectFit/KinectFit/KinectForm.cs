...
namespace KinectFit
{
    ...
    {
        private KinectSensor _Kinect; //Represents Xbox Kinect Sensor 
        private readonly Pen inferredBonePen = new Pen(Brushes.Gray, 1);//Color of bones when not they're not able to be tracked
        private readonly Pen trackedBonePen = new Pen(Brushes.Green, 6);//Color of bones when they're being tracked
        private readonly Pen trackedJointBrush = new Pen(Brushes.Red, 1);//Color of joints when they're being tacked
        private readonly Pen inferredJointBrush = new Pen(Brushes.Yellow, 1);//Color of joint when they're not able to be tracked
        private readonly Pen clippedBrush = new Pen(Brushes.Red, 12);//Color of bone when bone is being measured
        private Skeleton[] _FrameSkeletons;//Array of Skeletons that appear to the Kinect Sensor
        private ColorImageFormat imageFormat = ColorImageFormat.RgbResolution640x480Fps30;//RGB camera resoultion and frame rate
        private DepthImageFormat depthFormat = DepthImageFormat.Resolution640x480Fps30;//Depth camera resoultion and frame rate
        private Skeleton skeleton;//Represents the skeleton that the Kinect Sensor is looking at in the _FrameSKeletons array
        private Skeleton skeletonTracked = new Skeleton();//Represents the skeleton,user, being tracked by the Kinect Sensor
        private Int32 trackingID;//Holds value of skeleton being tracked
        private Boolean first = true;//We only need to track one person, I know we popluar and all but we can only handle one at a time even though the dream is more than one am right?

        private DepthImagePoint depthLeft;//Holds X,Y,Z values. Used to offset skeleton on the left side
        private DepthImagePoint depthRight;//Holds X,Y,Z values. Used to offset skeleton on the right side
        private Pen chestPen;//Used to draw parts of skeleton that are being measured by the Kinect Sensor

        private Double measurement = 0;//Holds value of skeleton part that is being measured
        private Double totalMeasurement = 0;//Holds final value of measured skeleton part that will be stored in the ConnectionClass object

        private Boolean isChest;//is the chest being measured?
        private Boolean isShoulder;//is the shoulder being measured?
        private Boolean isHip;//is the hip being measured?
        private Boolean isWaist;//is the waist being measured?
        private Boolean isLeg;//is the leg being measured?
        private Boolean part1Measured = false;//is first part being measured?
        private Boolean part2Measured = false;//is the second part being measured?
        private Boolean is2D;//NO 3D measurement being done but brands use full radius of X part to measure so there's a need to know when 2D math needs to be done.
        private String Part;//Which part is being measured? what is the name of that part? that value is stored here!

        private Boolean isReady = false;//are we ready to measure? no? yes? let me know
        ...
        Form7 frm7;//MeasureInfo object
        ConnectionClass cc;//KinectFit Connection class object
        public KinectForm(ConnectionClass _cc)
        {
            InitializeComponent();
            cc = _cc;
            frm7 = new Form7(cc);
        ...
        {
            cc.getParts();//Grab parts that need to be measured and store them in the ConnectionClass object
            ...
            DiscoverKinectSensor();//Lets find the Kinect!
        }
        ...
        private void DiscoverKinectSensor()
        {
            ...
            KinectSensor.KinectSensors.StatusChanged += KinectSensors_StatusChanged;//Status event 
            this.Kinect = KinectSensor.KinectSensors
                                      .FirstOrDefault(x => x.Status == KinectStatus.Connected);//Find the first Kinect and give me the status boss
        }

        private void KinectSensors_StatusChanged(object sender, StatusChangedEventArgs e)
        {
            //You guessed right it's the status switch
            switch (e.Status)
            {
        ...
        public KinectSensor Kinect
        {
            ...
            set
            {
                if (this._Kinect != value)//Only turn off the Kinect that wants to be turned off, that's what is happening here.
                {
        ...
        {
            if (sensor != null)//Do something only if it's on
            {

                sensor.AllFramesReady += Kinect_AllFramesReady;

                sensor.ColorStream.Enable(imageFormat);//We need the RGB camera!
                sensor.DepthStream.Enable(depthFormat);//We need the Depth camera!
                //Am I spelling camera right? I should probably google this...Yeah I google words.

                sensor.SkeletonStream.Enable();//Lets get that Skeleton ready!
                sensor.SkeletonStream.TrackingMode = SkeletonTrackingMode.Default;//Make sure you can keep track of him, can't afford to lose him/her.

                this._FrameSkeletons = new Skeleton[sensor.SkeletonStream.FrameSkeletonArrayLength];//How many people did you find? ok i'll make it that many for this array

                sensor.Start();//All good?Everything is enabled? ok let's start er up bud!

                if (sensor.ElevationAngle != 15)//Make sure upper body is always in view, 15 seems to work but this may change before demo day :O
        ...
        {
            if (sensor != null)//Don't turn it off it isn't on
            {
                sensor.Stop();
                sensor.AllFramesReady -= Kinect_AllFramesReady;//Remove event
                sensor.SkeletonStream.Disable();//Stop the skeleton
                this._FrameSkeletons = null;//Thank you for your work, rest now John.
            }
        }
        ...
        private void Kinect_AllFramesReady(object sender, AllFramesReadyEventArgs e)
        {
            ...
            if (colorFrame == null)//Something went wrong
            {
                return;//But don't worry we'll be back!
            }

            Bitmap bmpFrame = ColorImageFrameToBitmap(colorFrame);//Ok we need to create the live video feed for the user, this is step one in doing that

            using (SkeletonFrame frame = e.OpenSkeletonFrame())//Lets get that skeleton!
            {
                if (frame != null)//Make sure we actually got it
                {
                    Graphics g = Graphics.FromImage(bmpFrame);//Step two of getting that skeletong on that live video feed
                    JointType[] joints;//Holds all joints that are on a human.

                    frame.CopySkeletonDataTo(this._FrameSkeletons);//Lets copy that skeleton data you got for me since you work so much faster than I do :(
                    //The sensor found someone! quick store that in the skeleton.
                    this.skeleton = (from s in this._FrameSkeletons where s.TrackingState == SkeletonTrackingState.Tracked select s).FirstOrDefault();
                    if (this.skeleton == null)//Honestly, the first time you find someone this is still empty but I need that first person only!
                    {
                        if (this._Kinect.SkeletonStream.AppChoosesSkeletons)//Oh I gave you the power to track?
                        {
                            this.first = true;//Just by doing this!
                            this._Kinect.SkeletonStream.AppChoosesSkeletons = false;//You have lost your power.
                        }
                        return;//Once this is setup we'll be back with the real skeleton.
                    }

                    for (int i = 0; i < this._FrameSkeletons.Length; i++)
                    {
                        skeleton = this._FrameSkeletons[i];//Holds current skeleton the sensor is looking at.

                        if (this.skeleton.TrackingState == SkeletonTrackingState.Tracked)//The Kinect is tracking this right? ok cool
                        {
                            if (this.first)//This the first person?
                            {
                                this.skeletonTracked = this.skeleton;//Store that skeleton
                                this.trackingID = skeleton.TrackingId;//Get that ID we only need this skeleton
                                this._Kinect.SkeletonStream.AppChoosesSkeletons = true;//Hey man, think you can do the tracking for me? thanks.
                                    ...
                                    if (joint.JointType == JointType.Spine)
                                    {
                                        ...
                                        if (joint.Position.Z >= this.measureDistance && joint.Position.Z <= this.maxDistance)
                                        {
                                            if (!isReady)
                                            {
                                                ...
                                                isReady = true;
                                                timer1.Start();//Lets start measureing!
                                            }
                                        }
                                    }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if (this.timer1.Enabled)
            {
                ...
                {
                    if (this._Kinect.ElevationAngle != 15)
                    {
                        this._Kinect.ElevationAngle = 15;
                    }
                }
